...
from api.github.models import ContributionsModel, EventModel, EventsResponseModel, RepoModel
...
def get_events(response: Response):
    ...
    try:
        ...
        for event in raw_data:
            ...
            repos_seen.add(RepoModel(
                id=event['repo']['id'],
                name=event['repo']['name'],
                url=event['repo']['url'],
            ))
            if event['repo']['name'].startswith(Settings.GITHUB_USERNAME):
                contributions.own_projects += len(event['payload']['commits']) if 'commits' in event['payload'] else 1
